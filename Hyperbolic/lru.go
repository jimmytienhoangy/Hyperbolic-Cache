...
type LRU struct {
	...
	// total number of bytes the LRU can store
	max_capacity int

	// total number of current bytes in the LRU
...
func (lru *LRU) Get(key string) (access_count int, ok bool) {

	// get the list element associated with key
	_ , ok := lru.mapping[key]
...
func (lru *LRU) Set(key string, value []byte) bool {
	...
	if ok {
		...
		for lru.current_capacity > lru.max_capacity {
			...
			lru.current_capacity -= len((key_to_remove_test))
	...
	for insert_capacity > lru.RemainingStorage() {
		...
		// update the capacity
		lru.current_capacity -= len(value.(*list.Element).Value.(*binding).value)
		lru.current_capacity -= len((key_to_remove))
